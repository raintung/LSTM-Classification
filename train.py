...
def main():
    ...
    parser.add_argument('--utils_dir', type=str, default='pkls',
                        help='''directory containing pkls
    ...
    parser.add_argument('--vocab_size', type=int, default=392,
                        help='the vocab size')
...
def train(args):
    ...
    data_loader = DataLoader(True, args.utils_dir, args.data_path, args.batch_size, args.seq_length, None)
    ...
    with tf.Session() as sess:
        ...
        print("num epochs:%s"%(args.num_epochs))
        for e in range(args.num_epochs):
            sess.run(tf.assign(model.lr, args.learning_rate * (args.decay_rate ** e)))
            data_loader.reset_batch_pointer()
            print("num batches:%s"%(data_loader.num_batches))
            ...
            epochEnd = time.time()
            print 'epochs cost time {:.3f}'.format(epochEnd-epochStart)
